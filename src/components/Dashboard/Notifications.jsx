...
const Notifications = ({ token }) => {
  ...
  // without needing it in the dependency array (which caused reconnect loops)
...
socket.on("notification:new", (data) => {
  ...
  }, [token]); // `play` deliberately excluded — handled via ref above
    ...
    <>
      ...
      >
        ...
        {!loading && notificationCount > 0 && (
          <span
            aria-hidden="true"
            className="absolute -right-1 -top-1 flex size-4.5 items-center justify-center
                                   rounded-full bg-red-500 text-[11px] font-bold leading-none
                                   text-white border-white border-[2px]"
          >
